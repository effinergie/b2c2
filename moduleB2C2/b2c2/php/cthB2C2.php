...
class CthB2C2 extends FdrB2C2{
	...
	**************************************/	
	public function cth(){	
		...
		try {			
			...
			$this->calcResult['recapCriteres']=$this->cth_recapCriteres();
			$this->calcResult;
			return $this->calcResult;
		} catch (Throwable $e) {
			$data = $e->getTraceAsString();
			$this->trace("#### ERREUR #### : " . $e->getMessage()."<br><pre>\n" .$data .'</pre>');
			//echo ("#### ERREUR #### : " . $e->getMessage()."<br><pre>\n" .$data .'</pre>');
			return [];
		}	
	}
	...
	public function cth_conso($nEtap){
		...
		$cepVentilation = $this->cth_CepVentilation($nEtap);

		$cef = $consoChauffage['cef'] + $consoECS['cef'] + $cepClim['cef'] + $cepEclairage['cef'] + $cepVentilation['cef'];
		$cep = $consoChauffage['cep'] + $consoECS['cep'] + $cepClim['cep'] + $cepEclairage['cep'] + $cepVentilation['cep'];
		$ges = $consoChauffage['ges'] + $consoECS['ges'] + $cepClim['ges'] + $cepEclairage['ges'] + $cepVentilation['ges'];
				
		$this->trace('cth_conso() : $cep = $consoChauffage[cep] + $consoECS[cep] + $cepClim[cep] + $cepEclairage[cep] + $cepVentilation[cep]; ','hl');
		$this->trace("cth_conso() : $cep = $consoChauffage[cep] + $consoECS[cep] + $cepClim[cep] + $cepEclairage[cep] + $cepVentilation[cep]; ",'hl');
	...
	**************************************/		
	protected function cth_consoChauffage($nEtap){
		...
		$this->calcResult['etape'][$nEtap]['consoLot']['chauffage'] = $consoChauffageTotal;
	...
	protected function cth_consoChauffageType_detail($bCh,$typeChauffage,$nEtap){
		...
		$ich = 0;
		//$valChamp = $this->getTypeLotSolution($typeChauffage)['resEmetteurChauffage'][0];	
		if ($Rr*$Rd*$Re*$Rg){
			$ich = 1/($Rr*$Rd*$Re*$Rg);			
		}
	...
	protected function cth_consoChauffageAppoint($bCh,$typeChauffage,$nEtap,&$consoChauffPrinc){
		...
		if ($appoint){
			$aEppoint = explode('|',$appoint);
			$prodAppoint = $aEppoint[0];
			$partAppoint = $this->floatvalFr($aEppoint[1]);			
	...
	protected function cth_Rg_Chaufage($typeChauffage,$nEtap){
		$nomChampRg = ($this->isTypeBatLC()? 'RgChaufCol' : 'RgChaufInd');
		$valRg = $this->paramChampAvantApres($typeChauffage,$nEtap,'typeProdChauffage',$nomChampRg,'resTypeProd',$nomChampRg);
	...
	protected function cth_Rr_Chaufage($typeChauffage,$nEtap){
		$nomChampRrApres = ($typeChauffage['existant']['chauffageCollectif'] == 'oui')? 'RrChCol' : 'RrChInd';
		return $this->floatvalFr($this->paramChampAvantApres($typeChauffage,$nEtap,'typeEmmeteurChauffage','Rr','resTypeProd',$nomChampRrApres));
	}		
	protected function cth_Re_Chaufage($typeChauffage,$nEtap){
		$nomChampReApres = ($typeChauffage['existant']['chauffageCollectif'] == 'oui')? 'ReChCol' : 'ReChInd';
		return $this->floatvalFr($this->paramChampAvantApres($typeChauffage,$nEtap,'typeEmmeteurChauffage','Re','resTypeProd',$nomChampReApres));
	}		
	protected function cth_Rd_Chaufage($typeChauffage,$nEtap){
		$champRd = ($typeChauffage['existant']['chauffageCollectif'] == 'oui') ? 'RdChCol' : 'RdChInd';
		return $this->floatvalFr($this->paramChampAvantApres($typeChauffage,$nEtap,'typeEmmeteurChauffage',$champRd,'resTypeProd',$champRd));
	}		
	...
	protected function cth_BchMoi($typeChauffage,$nEtap,$iMoi){
		...
		//$this->trace ('cth_BchMoi($iMoi) : $inertieLourde ');
		//$this->trace ("cth_BchMoi($iMoi) : $inertieLourde ");
	...
	public function cth_I0($typeChauffage,$nEtap){	
		if(!$this->isTypeBatLC()){
			...
			//LC
			$nomChampRg = ($typeChauffage['existant']['chauffageCollectif'] == 'oui')? 'I0ColChCol' : 'I0ColChInd';
		}
	...
	**************************************/		
	protected function cth_consoEcs($nEtap){
		...
		$this->calcResult['etape'][$nEtap]['consoLot']['ecs'] = $consoEcsTotal;
	...
	protected function cth_consoEcsType($typeLot,$nEtap){
		...
		/*$Qgw = 0;//todo :  Fluo CalculThermique!578
		$Rd = 0.52;//todo :  Fluo CalculThermique!581
		$Rs = 1/(1+($Qgw*$Rd)/ $bEcsTotal );//todo :  Fluo CalculThermique!580		
		$Rg = 0.9;//todo : Fluo CalculThermique!582
		$Iecs = 1 / ($Rs * $Rd * $Rg);*/
	...
	protected function cth_Fecs($typeLot,$nEtap){		
		if ($this->ecsConnectChauff($typeLot,$nEtap)){
			$typeChauffage = $this->getTypeLotPrincipal('chauffage');
			$presenceSolaire = $this->paramChampAvantApres($typeChauffage,$nEtap,'typeProdChauffage','solaire','resTypeProd','solaire');
		} else {	
	...
	**************************************/		
	...
	/*	Julien a dit  : la clim c'est toujours 0
		$zoneClimEte = $this->getZoneClimEte();	
		$SClim = $this->cth_Sclim();
		
		if ($this->isTypeBatLC()){	
			$niv = $this->jData['projet']['batiment']['nbNiveau'];
			$SClimAutre	= ($niv-1)/$niv*$SClim;
			$SClimDernier = $SClim-$SClimAutre;
			$RclimDernier 	= $this->floatvalFr($this->getColTableauValeur('zoneClimEte','id',$zoneClimEte,'rClimColDernierEtage'));
			$RclimAutre 	= $this->floatvalFr($this->getColTableauValeur('zoneClimEte','id',$zoneClimEte,'rClimColAutreEtage'));
			
			$Cclim = $RclimDernier * $SClimDernier + $RclimAutre * $SClimAutre;
			$this->trace ('cth_CEPClim() : $Cclim = $RclimDernier * $SClimDernier + $RclimAutre * $SClimAutre;');
			$this->trace ("cth_CEPClim() : $Cclim = $RclimDernier * $SClimDernier + $RclimAutre * $SClimAutre;");			
		} else {			
			if ($SClim<150){
				$nomColRclim = 'rClimIndInf150' ;		
			} else {
				$nomColRclim = 'rClimIndSup150'	;
			}	
			$this->trace ("cth_CEPClim() : getColTableauValeur('zoneClimEte','id',$zoneClimEte,$nomColRclim)");	
			$Rclim = $this->floatvalFr($this->getColTableauValeur('zoneClimEte','id',$zoneClimEte,$nomColRclim));
			
			$Cclim = $Rclim * $SClim;			
			$this->trace ('cth_CEPClim() : $Cclim = $Rclim * $SClim;');	
			$this->trace ("cth_CEPClim() : $Cclim = $Rclim * $SClim;");				
		}	
		...
		$CEPClim = $facteurEfEp * $Cclim / $shab;
		$gesClim = $facteurGes * $Cclim / $shab;
		$this->trace ('cth_CEPClim() : $CEPClim = $facteurEfEp * $Cclim / $shab;');	
		$this->trace ("cth_CEPClim() : $CEPClim = $facteurEfEp * $Cclim / $shab;");
		return [
			'cep'=>$CEPClim,
			'ges'=>$gesClim
	...
	**************************************/		
	...
	/*
		...
		if ($this->travauxEffectues($typeLot,$nEtap) && $this->getTypeLotSolution($typeLot)['resTypeVentilation']=="naturelleAssit"){
			if($this->isTypeBatLC()){
				$PventColl_m2 = $this->floatvalFr($this->getColTableauValeur('resTypeVentilation','id',"auto",'PventCol_m2'));
				$Qvarepconv = $this->floatvalFr($this->getColTableauValeur('resTypeVentilation','id',"auto",'QvarepconvCol'));
				$PventMoy = $shab * $PventDefaut * $Qvarepconv;					
			} else {
				$PventMoy = $this->floatvalFr($this->getColTableauValeur('resTypeVentilation','id',"auto",'PventInd')); 			
			}
			//$hGrandDebit = $this->floatvalFr($this->cth_paramBat('typeBat','dureeGrandDebit'));
			$ratioTemps = $this->floatvalFr($this->cth_paramBat('typeBat','ratioTmpVentMeca'));
	...
	*******************************/
	...
	public function cth_uMurExt($typeMur,$nEtap){
		...
		//rIsoAvant
		$resistMur = $typeMur["existant"]["resistMur"];	
		$rIsoAvant = $this->floatvalFr($this->getColTableauValeur('resistMur','id',$resistMur,'r'));			
		...
		if ($this->travauxEffectues($typeMur,$nEtap)){
			$rIsoApres = $this->floatvalFr($typeMur["lstSol"][$idSol]["perfMin"][0]);
		}
	...
	public function cth_uPlancherHaut($typeLot,$nEtap){		
		...
		//rIsoAvant
		$resistPlancherHaut = $typeLot["existant"]["resistPlancherHaut"];	
		$rIsoAvant = $this->floatvalFr($this->getColTableauValeur('resistPlancherHaut','id',$resistPlancherHaut,'r'));			
		//if (!$rIsoAvant && $typeLot["existant"]["typeIsoMur"] != "non"){ //pas de champ permettant de savoir sur présence isolan avant. Comme il y a deja un champ "résistance avant, je trouve ça inutile ou bien mettre un niveau 0"
		//	$rIsoAvant = 0.5;
		//}	
		...
		if ($this->travauxEffectues($typeLot,$nEtap)){
			$rIsoApres = $this->floatvalFr($typeLot["lstSol"][$idSol]["perfMin"][0]);
		}
	...
	public function cth_uPlancherBas($typeLot,$nEtap){		
		...
		//rIsoAvant
		$resistPlancherBas = $typeLot["existant"]["resistPlancherBas"];	
		$rIsoAvant = $this->floatvalFr($this->getColTableauValeur('resistPlancherBas','id',$resistPlancherBas,'r'));			
		//if (!$rIsoAvant && $typeLot["existant"]["typeIsoMur"] != "non"){ //pas de champ permettant de savoir sur présence isolan avant. Comme il y a deja un champ "résistance avant, je trouve ça inutile ou bien mettre un niveau 0"
		//	$rIsoAvant = 0.5;
		//}	
	...
	public function cth_surfMenuiserieExt($bSeulementVerticale= false){
		...
		/*if ($res = $this->jData['projet']['batiment']['surfMenuiserie']) {
			return $res;
		}	
		
		$taux = 0.1;//au cas ou, normalement pas de calcul auto
		$res = $this->cth_surfMurExt() * $taux;*/
	...
	public function cth_uMenuiseriesExt($typeLot,$nEtap){	
		...
		//uwAvant
		$perfMenuiserie = $typeLot["existant"]["perfMenuiserie"];	
		$uwAvant = $this->floatvalFr($this->getColTableauValeur('perfMenuiserie','id',$perfMenuiserie,'uw'));			

		//uwApres
		$uwApres= 0;
		if ($this->travauxEffectues($typeLot,$nEtap)){
			$uwApres = $this->floatvalFr($typeLot["lstSol"][$idSol]["perfMin"][0]);
		}
	...
	public function cth_coefPontsThermiques_methodeSimpl($typeMur,$nEtap){		
		...
		/*$resistMur = $typeLot["existant"]["resistMur"];	
		$rmMurAvant = $this->floatvalFr($this->getColTableauValeur('resistMur','id',$resistMur,'r'));
		//si mur suffisament isolé à l'etat initial, on prend un autre coef
		if ($rmMurAvant>=3.7){
			$typeIsoMur = $typeLot["existant"]["typeIsoMur"];	
			$coefPTAvant = $this->floatvalFr($this->getColTableauValeur('typeIsoMur','id',$typeIsoMur,'coefPT'));
		}
		
		//uApres
		$coefPTApres=0;
		if ($this->travauxEffectues($typeMur,$nEtap)){			
			$resTypeIsoMur =$typeLot["lstSol"][$idSol]["resTypeIsoMur"];
			$coefPTApres = $this->getSommeColTableauListeValeur('resTypeIsoMur','id',$resTypeIsoMur,'coefPT');			
		}
	...
	protected function cth_permea_final(){	
		...
		if (!$Q4paconv){				
			$Q4paconvMin = 1.7;
		//dans la V15 on ne minore plus par 1.3 si ITI ou planchers pas traité..... etrange
		//	if ($this->partMursITI($nEtap)>=50 OR $this->partPlancherBasTraite($nEtap)<50){
		//		$Q4paconvMin = 1.3;
		//	}
		//	$this->trace ('cth_permea() : $Q4paconvMin');
		//	$this->trace ("cth_permea() : $Q4paconvMin");
			//apres trav
	...
	protected function cth_criteresMaPrimeRenov(){
		$res = [];
		$nbEtape = count($this->calcResult['etape']);
		foreach ($this->calcResult['etape'] as $nEtap=>$aCalculs){
			$rapShonShab = $this->cth_shon() / $this->cth_shab();
			//echo '<pre>';print_r($aCalculs);die;			
	...
	protected function cth_niveauDpeCep($cep){	
		$aEtiq = $this->getListeValeurDepuisTableau('dpeCep','id','valMax');
		foreach ($aEtiq as $etiq=>$valMax){
			if ( $valMax and $cep<=$valMax){
				return $etiq;
			}
		}
		return $etiq;
	...
	protected function cth_niveauDpeGes($cep){	
		$aEtiq = $this->getListeValeurDepuisTableau('dpeGes','id','valMax');
		foreach ($aEtiq as $etiq=>$valMax){
			if ( $valMax and $cep<=$valMax){
				return $etiq;
			}
	...
	protected function cth_criteresCeeBar164(){
		$res = [];
		$nbEtape = count($this->calcResult['etape']);
		foreach ($this->calcResult['etape'] as $nEtap=>$aCalculs){
			//echo '<pre>';print_r($aCalculs);die;			
